...
import { AddToExistedResult, SetSearchResult } from '../Redux-Toolkit/Slices/SearchResult';

export default function usePaginatedSearch() {
    const dispatch=useDispatch()
    const sendReq=useSendAuthRequest()
    async function fetchData(searchKey,searchValue,pathAfterApi,sessionStorageKey,pageNum) {
        window.sessionStorage.setItem(sessionStorageKey, pageNum);
        const result = await sendReq(pathAfterApi + `?searchKey=${searchKey}&searchValue=${searchValue}&page=${pageNum}`, "get");
    ...
    return async function (searchKey,searchValue,pathAfterApi,sessionStorageKey) {
       ...
       window.onscroll=async function (){
        ...
        if (window.scrollY >= document.body.scrollHeight - window.innerHeight) {
          ...
          await fetchData(searchKey,searchValue,pathAfterApi,sessionStorageKey,+currentPage + 1);
        }
